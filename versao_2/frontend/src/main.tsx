...
function TseaTwinRecoveryPanel({ state, allTanks, allHoses }: any) {
  ...
  return (
    <Section title="Simulação Completa — cenários e testes" subtitle="Cenários base, personalizados, criação de teste e resultado com rastreabilidade completa.">
...
function TseaDigitalTwin10({ state, allTanks, allHoses }: any) {
  ...
  return (
    <Section title="Simulação Completa do processo de vácuo" subtitle="Simulação operacional com cenários, criação de testes, diagnóstico, rastreabilidade e histórico técnico.">
...
function tseaHRBuildWordRecord(record: any, kind: string, allTanks: any[], allHoses: any[]) {
  ...
  return `
    <div class="cover">
      ...
      <p><strong>Sistema:</strong> Rastreabilidade e Simulação Completa do processo de vácuo</p>
...
function tseaHRBuildWordGeneral(operations: any[], simulations: any[]) {
  ...
  return `
    <div class="cover">
      ...
      <p><strong>Sistema:</strong> Rastreabilidade, operação e Simulação Completa do processo de vácuo</p>
      <p><strong>Data de emissão:</strong> ${tseaHRDate()}</p>
      <div class="bottom">
        <p>Belo Horizonte</p>
        <p>${new Date().getFullYear()}</p>
    ...
    <div class="page-break sumario">
      ...
      <p>4. Simulações do Simulação Completa</p>
    ...
    <div class="page-break">
      <h1>1. Introdução</h1>
      <p>
        Este relatório consolida registros operacionais e simulações executadas no TSEA V-Twin · Protótipo Físico,
      ...
      <h1>4. Simulações do Simulação Completa</h1>
      ...
      <p>
        ...
        melhoria contínua e evolução do Simulação Completa para integração com dados reais da linha de produção.
      </p>
    </div>
  `;
}
...
function TseaHistoryMenuV2({ operations = [], state, allTanks = [], allHoses = [] }: any) {
  ...
  return (
    <div className="hrMenu">
      <Section title="Histórico técnico" subtitle="Consulta organizada de operações reais e simulações do Simulação Completa.">
...
function TseaReportsMenuV2({ operations = [], state, allTanks = [], allHoses = [] }: any) {
  ...
  return (
    <div className="hrMenu">
      ...
      {tab === "overview" && (
        <Section title="Visão geral dos relatórios" subtitle="Resumo dos registros disponíveis para exportação.">
          <div className="metrics">
            ...
            <Metric label="Simulações filtradas" value={filteredSimulations.length} detail="Simulação Completa" />
      ...
      {tab === "simulations" && (
        <Section title="Relatório de simulações" subtitle="Simulações executadas no Simulação Completa com status, risco e pressão final.">
          <Table
            columns={["ID", "Data", "Cenário", "Status", "Risco", "Pressão", "Ações"]}
