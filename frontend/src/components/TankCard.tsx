  ...
  return (
    <article className={`tank-card ${item.status_light}`}>
      ...
      <div className="tank-metrics">
        ...
        <Metric label="Mangueira" value={item.hose?.code ?? "--"} />
        ...
        <Metric label="Risco de colapso" value={fmt(item.collapse_risk_pct, "%")} />
        <Metric label="Status" value={item.status_light === "green" ? "Normal" : item.status_light === "yellow" ? "Atenção" : "Crítico"} />
      </div>
    </article>
  );
}
